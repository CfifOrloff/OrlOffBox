...
owm = pyowm.OWM('999d132af8a42fbb091816af70efe1b8', config_dict)
place = 'Kimry'  # ('Родной город Бота "Бро" ')
mgr = owm.weather_manager()
observation = mgr.weather_at_place(place)
weather = observation.weather
temp = weather.temperature('celsius')['temp']
status_t = weather.status
status = weather.detailed_status
status_ico = ()


one = types.ReplyKeyboardMarkup(row_width=2)
button1 = types.KeyboardButton(" Тут стоят")
button2 = types.KeyboardButton(" Музыка")
button3 = types.KeyboardButton(" Погода")
button4 = types.KeyboardButton(" Новости")
one.add(button1,button2,button3,button4)


one_in = types.InlineKeyboardMarkup(row_width=2)
button_1 = types.InlineKeyboardButton(" Тут стоят!", callback_data='location')
button_2 = types.InlineKeyboardButton(" Где стоят?", callback_data='gde')
one_in.add(button_1,button_2)
...
def get_weather():
	if status_t == 'Clear':
		status_ico = (" ☀️ ️")
	elif status_t == 'Clouds':
		status_ico = (" ⛅️")
	else:
		status_ico = (" 🌧 ")
	tempall = str(' | ' + str(int(temp)) + '°' + str(status_ico) )
...
def get_weather_all():
	if status_t == 'Clear':
		...
		status_ico = (" 🌧 ")
	tempall_send = str( str(int(temp)) + '°' + str(status_ico) + ' ' + str(status) + ' ' + str(weather.temp) )
	return tempall_send
...
def transferMessagesToTelegram( idd, userName, mbody, fwdList ):
	...
	if not fwdList is None:
		...
		module.bot.send_message( config.getCell( 'vk_' + idd ), niceText, str(reply_markup=one_in) )
...
def listener( messages ):
	for m in messages:
		...
		if m.content_type == 'text':
			...
			if m.text == '/start':
				module.bot.send_message( m.chat.id, "Привет Бро", reply_markup=one_in)
				if m.text == 'где':
					module.bot.send_location( m.chat.id, lat, lon)
				continue
			if m.text == 'погода':
				tempall_send = get_weather_all()
				module.bot.send_message( m.chat.id, str( tempall_send ), reply_markup=one_in)
				continue
		...
		checkRedirect_telegram( str( m.chat.id ), str( m.text), getUserTName(m.from_user), str(filePath), reply_markup=one_in)
def init_telegram():
